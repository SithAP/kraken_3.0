...
from geometry_msgs.msg import TwistWithCovarianceStamped as DVL_Msg
...
pub = rospy.Publisher(topicHeader.SENSOR_DVL,DVL_Msg, queue_size = 2)
rospy.init_node('DVL_Data_Publisher', anonymous=True)
...
rawData = ''
pubData =  DVL_Msg()
pubData.header.frame_id='dvl'
if rospy.has_param('DVL_Uncertainity'):
	pubData.twist.covariance = map(float,rospy.get_param('DVL_Uncertainity')[1:-1].split(','))
else:
	pubData.twist.covariance = [-1.0] * 36
## DVL config
dvl.stopbits = 2
##
